function MPC_out = MPC_TO_dri_phase2( Y_ego, vy_ego, fai_ego, dfai_ego, Y_desire, fai_desire, oldu1, delta_ego, ddelta_ego, sin_fai, cos_fai, scale)
...
if isempty(aa)
    ...
    t_fai = 21*(1/0.01)^2;
    t_trq = .1*(1/20)^2;
    t_dtrq = .00001*1.26*(1/8/T_s)^2;
%     t_dtrq = .00001*1.28*(1/8/T_s)^2;
    ...
    for k=1:N
        ...
        alfa_f{k} = (xvar{k}(1) + lf*xvar{k}(2))/vx - xvar{k}(6)/is*scale_para;
    ...
    parameters_in = {xvar{1},Y_tgt,yaw_angle_tgt,pastu1,scale_para};%,pastu2
...
%     solutions_out = uvar{1};
    ...
    [MPC_out,diagnostic,~,~,controller] = controller{{[vy_ego; dfai_ego; Y_ego; sin_fai; cos_fai; delta_ego; ddelta_ego; fai_ego],Y_desire,fai_desire,oldu1,scale}};%,ax_ego
    ...
    %     assign(uvar{1},delta_ego);
    [MPC_out,diagnostic,~,~,controller] = controller{{[vy_ego; dfai_ego; Y_ego; sin_fai; cos_fai; delta_ego; ddelta_ego; fai_ego],Y_desire,fai_desire,oldu1,scale}};%,ax_ego,vx_ego
